...
/// paridad exacta de campos con InformacionColaborador (PublicEvents). No lo referencia --
/// Programacion.DomainEvents queda con cero ProjectReference (CA-ADR-0029 enmendado por #317). Lo
/// usa ProgramacionTurnoSolicitada, el evento que SE PERSISTE.
///
...
/// ColaboradorProgramado de ControlHoras.DomainEvents: mismo nombre en otra isla, sin referencia
/// entre ambos (patron SedeProgramada, #336). Ese rename de #340 no toco ninguna clave JSON.
///
/// Issue #433 (fase A): la paridad de campos con ese gemelo se ROMPIO a proposito -- el de
/// ControlHoras ya es la terna de identidad y este sigue siendo el quinteto, porque la reduccion
/// solo puede avanzar del final de la cadena hacia el principio (la terna se deriva del quinteto,
/// no al reves). La fase B reduce este tipo y el body HTTP, y restablece la simetria.
///
...
/// del record por defecto ya es correcta (mismo criterio que InformacionColaborador, issue #318).
/// </remarks>
