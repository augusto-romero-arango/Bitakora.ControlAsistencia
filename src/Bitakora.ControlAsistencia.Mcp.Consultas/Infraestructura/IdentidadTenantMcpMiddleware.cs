...
// TenantContextMiddleware de los dominios. Lee el Authorization del ToolInvocationContext y no de
// FunctionContext.GetHttpContext(): una invocacion por McpToolTrigger no llega al worker con
// HttpContext -- el endpoint del protocolo lo sirve el paquete del host (ver
// AutorizacionMcpMiddleware, "LIMITE ESTRUCTURAL").
//
// Sin verificar en runtime: que el host propague el Authorization del cliente MCP dentro de
// HttpTransport.Headers. Si lo filtra, la identidad ambiente queda sin poblar y el propagador cae
    ...
    IValidadorTokenAuthKit validador, IDerivadorIdentidadTenantMcp derivador) : IFunctionsWorkerMiddleware
{
        ...
        string? encabezadoAutorizacion, CancellationToken cancellationToken = default)
    {
        ...
        return derivador.Derivar(await validador.ValidarAsync(token, cancellationToken));
    }
