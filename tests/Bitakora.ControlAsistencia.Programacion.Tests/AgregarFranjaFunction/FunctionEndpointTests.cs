// Issue #602: tests del endpoint HTTP POST /programacion/turnos/{id}:agregar-franja
...
public class FunctionEndpointTests
{
    ...
    private static AgregarFranjaBody BodyValido() =>
    ...
    // CA-7 (ultimo enunciado): la ruta declarada es exactamente esta, con verbo post -- congelada
    // por reflexion (mismo criterio que ContratoHttpComandosTests de Colaboradores), porque
    // ningun otro test local ejercita el HttpTriggerAttribute (Run() se llama directo, sin pasar
    // por el enrutador del host).
    [Fact]
    public void AgregarFranja_ExponeElVerboYLaRutaPactadosEnElIssue()
    {
        var trigger = Trigger();
    ...
    [Fact]
    public async Task AgregarFranja_Retorna400_CuandoElIdDeRutaNoEsUnGuidValido()
    {
        var validator = new FakeRequestValidator<AgregarFranjaBody>(BodyValido());
        var router = new FakeCommandRouter();
        var function = new FunctionEndpoint(validator, router);
    ...
    [Fact]
    public async Task AgregarFranja_Retorna400_CuandoElBodyNoValida()
    {
        var error = new BadRequestObjectResult("El body es invalido o esta malformado");
        var validator = new FakeRequestValidator<AgregarFranjaBody>(error: error);
        var router = new FakeCommandRouter();
        var function = new FunctionEndpoint(validator, router);
    ...
    // Invariante del VO (FranjaOrdinaria.Crear): el handler deja subir la ArgumentException sin
    // envolverla, el endpoint la traduce a 400 con mensaje.
    [Fact]
    public async Task AgregarFranja_Retorna400ConMensaje_CuandoElRouterLanzaArgumentException()
    {
        var validator = new FakeRequestValidator<AgregarFranjaBody>(
            new AgregarFranjaBody(new TimeOnly(10, 0), new TimeOnly(10, 0)));
        var router = new FakeCommandRouter(
            new ArgumentException("La duracion de la franja debe ser positiva"));
        var function = new FunctionEndpoint(validator, router);
    ...
    [Fact]
    public async Task AgregarFranja_Retorna404_CuandoElTurnoNoExiste()
    {
        var validator = new FakeRequestValidator<AgregarFranjaBody>(BodyValido());
        var router = new FakeCommandRouter(
    ...
    [Fact]
    public async Task AgregarFranja_Retorna409_CuandoLaFranjaSeSolapaConOtraExistente()
    {
        var validator = new FakeRequestValidator<AgregarFranjaBody>(
