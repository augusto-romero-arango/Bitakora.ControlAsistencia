...
public class DescansoQuitadoSerializacionTests
{
    ...
    [Fact]
    public void Deserializar_ReconstruyeEvento_CuandoFranjaResultanteYaNoTraeElDescanso()
    {
        var franja = FranjaOrdinaria.Crear(new TimeOnly(22, 0), new TimeOnly(6, 0))
            .ConDescanso(new TimeOnly(2, 0), new TimeOnly(2, 30));
        var evento = DescansoQuitado.Crear(TurnoId, franja);

        var opciones = CrearOpcionesMarten();
        var json = JsonSerializer.Serialize(evento, opciones);
        var deserializado = JsonSerializer.Deserialize<DescansoQuitado>(json, opciones);
    ...
    public void Deserializar_Falla_CuandoResolverNoTieneRegistroDeDescansoQuitado()
    {
        ...
        var act = () => JsonSerializer.Deserialize<DescansoQuitado>(json, opcionesSinResolver);
