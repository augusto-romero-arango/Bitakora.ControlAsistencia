...
// Issue #183: el evento ya no transporta el DesgloseHoras rico sino su discriminacion primitiva
//            (HorasDiscriminadas, via DesgloseHoras.Discriminar()). Estos tests verifican entonces dos
//            cosas complementarias: (a) que el aggregate consolida el desglose REAL (propiedad
//            DesgloseHoras, rica) y (b) que CrearDiaCalculado() lleva ESE desglose ya discriminado al
//            payload, no un payload vacio. El conteo de FranjasAnomalas vive solo en (a): por diseno del
//            issue #183 no viaja en el contrato primitivo.
//
...
public class CrearDiaCalculadoConDesgloseRealTests
{
        ...
        : CommandHandlerAsyncTest<ProgramacionTurnoDiarioSolicitada>
    {
        ...
        public async Task CrearDiaCalculado_LlevaDesgloseRealIgualAlAggregate_CuandoFranjaNoEsAnomala()
        {
            ...
            // Issue #183: el evento empaquetado por CrearDiaCalculado() lleva el desglose real ya
            // DISCRIMINADO al payload primitivo (oraculo no vacio => no es el equivalente discriminado de
            // DesgloseHoras.Vacio). El desglose real (ordinaria 07:00-14:00 DominicalFestivaDiurna = 420min,
            // retardo neto 0) discrimina a un unico concepto. Oraculo independiente construido a mano.
            ...
            // ...y la propiedad DesgloseHoras del aggregate sigue exponiendo ESE mismo desglose real RICO
            // (el que se discrimina al construir el evento): luego el evento refleja el estado real del
            // aggregate, no DesgloseHoras.Vacio.
        ...
        public async Task CrearDiaCalculado_LlevaDesgloseConFranjasAnomalas_CuandoTurnoSinMarcaciones()
        {
            ...
            // Issue #183: el contrato primitivo NO transporta FranjasAnomalas; una franja anomala no
            // aporta conceptos ni retardo, asi que el evento discriminado queda vacio.
            And<ControlDiarioAggregateRoot, HorasDiscriminadas>(
                StreamId,
                c => c.CrearDiaCalculado().HorasDiscriminadas,
                new HorasDiscriminadas(new Dictionary<string, int>(), []));

            // El conteo de anomalas (=1, que distingue este caso de DesgloseHoras.Vacio con FA=0) sigue
            // vivo en la propiedad DesgloseHoras RICA del aggregate, que es la que se discrimina al evento.
            And<ControlDiarioAggregateRoot, DesgloseHoras>(
                StreamId,
                c => c.DesgloseHoras,
        ...
        : CommandHandlerAsyncTest<MarcacionRegistrada>
    {
        ...
        public async Task CrearDiaCalculado_LlevaDesgloseVacio_CuandoNoHayTurno()
        {
            ...
            // Issue #183: sin turno el desglose consolidado es DesgloseHoras.Vacio y discrimina a vacio.
            And<ControlDiarioAggregateRoot, HorasDiscriminadas>(
                StreamId,
                c => c.CrearDiaCalculado().HorasDiscriminadas,
                new HorasDiscriminadas(new Dictionary<string, int>(), []));

            // La propiedad DesgloseHoras del aggregate se preserva en DesgloseHoras.Vacio (caso vacio).
            And<ControlDiarioAggregateRoot, DesgloseHoras>(
                StreamId,
                c => c.DesgloseHoras,
                DesgloseHoras.Vacio);
