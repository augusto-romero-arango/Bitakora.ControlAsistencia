...
| **Turno componible y editable: nace vacío, `esDescanso` explícito en evento y aggregate, turno incompleto vs descanso, programable = completo, comandos de diseño `:verbo` por hora de inicio, eventos de diseño con la franja completa, offsets de hijas inferidos, ordinaria ≤ 24 h inclusive** | CA-ADR-0033 |
| **Plantilla semanal de turnos: segundo nivel de composicion (1..N semanas lunes-domingo), dias por (semana, diaSemana) con referencia a `TurnoId` (nunca snapshot), descanso es turno, dia = slot atomico PUT/DELETE, plantilla incompleta al retirar un turno, vista `CuadroSemanalTurnos` N2 con grouper; "Ficha" no es patron de naming; ciclos 4x2/2x2 fuera** | CA-ADR-0034 |
...
| **Identidad ambiente AsyncLocal (`TenantExecutionContext`/`TenantContextMiddleware`) que reemplaza a `AgregarTenantResolverHibrido()`/`ProxyTenantResolver` en el worker aislado; por que ese resolver del marco falla ahi** | CA-ADR-0032 (desviacion de MEF-ADR-0028 seccion 4; propuesto al marco) |
