...
| `programacion` | `CatalogoTurnos`, `SolicitudProgramacionAggregateRoot`, `SolicitudCancelacionAggregateRoot` (nace en #498) | Guid canonico "D" | Sin cambio | Paso 1 de la heuristica: ninguno de los tres necesita identidad natural. Sin registro adicional -- un Guid nunca colisiona con otro Guid de otro aggregate por construccion (espacio de valores, no de forma). |
