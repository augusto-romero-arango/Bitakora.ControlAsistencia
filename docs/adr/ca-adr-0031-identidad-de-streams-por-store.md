...
| `programacion` | `CatalogoTurnos`, `SolicitudProgramacionAggregateRoot`, `SolicitudCancelacionAggregateRoot` (nace en #498), `PlantillaSemanalTurnos` (nace en #620) | Guid canonico "D" | Sin cambio | Paso 1 de la heuristica: ninguno de los cuatro necesita identidad natural. Sin registro adicional -- un Guid nunca colisiona con otro Guid de otro aggregate por construccion (espacio de valores, no de forma). `PlantillaSemanalTurnos` (#620) entra por la misma via: `Id = evento.PlantillaId.ToString()`, con el `PlantillaId` que genera el cliente. |
...
- 2026-08-29: enmienda del registro (paso 6 de la heuristica) por el issue #498 -- nace
  `SolicitudCancelacionAggregateRoot` en el store `programacion`, con la misma anatomia que
  `SolicitudProgramacionAggregateRoot` (Guid canonico "D", paso 1: no necesita identidad natural --
  la idempotencia la da el `Id` que el cliente asigna a la solicitud). No hay colision que dirimir:
  dos Guid no colisionan por construccion (espacio de valores, no de forma), asi que la fila del
  store solo suma el aggregate a la enumeracion.
