# ADR-0021: Snapshots de Marten como excepción, no como regla
...
- **Un aggregate puntual podría crecer más rápido de lo previsto**: si el crecimiento lleva a latencias problemáticas antes de que se detecte, algunos comandos pueden tardar más de lo aceptable durante un período. Mitigación: telemetría de `App Insights` en los handlers permite detectarlo rápidamente (ver ADR-0009 sobre control de costos y uso de Application Insights). Umbral de alerta propuesto: rehidratación de aggregate > 500ms p95.

## Referencias

- ADR-0006: Event sourcing con Marten + Wolverine (define el stack; este ADR restringe una capacidad de Marten específicamente).
- ADR-0009: Control de costos Application Insights (la telemetría que permite detectar el momento en que la excepción se justifique).
- ADR-0015: Estilo de modelado de objetos de dominio (explica el enfoque de aggregates ricos y cómo exponen estado).
