...
# MEF-ADR-0020 (sku_name, os_type, worker_count, always_on -- issue #400): os_type y worker_count
# no se pasan explicitos porque sus defaults ("Linux", 1) replican el comportamiento previo sin
# diff. Mismo patron que dominio-colaboradores.tf.

resource "random_string" "storage_suffix_sedes" {
  length  = 6
  special = false
  upper   = false
}

module "storage_sedes" {
  source              = "../../modules/storage"
  name                = "stsedes${var.environment}${random_string.storage_suffix_sedes.result}"
...
module "function_app_sedes" {
  source                         = "../../modules/function-app"
  name                           = "func-${local.prefix_short}-sedes"
  resource_group_name            = module.resource_group.name
  location                       = module.resource_group.location
  service_plan_id                = module.service_plan_sedes.id
...
# settings.
resource "azurerm_role_assignment" "kv_secrets_user_sedes" {
  scope                = module.key_vault.id
  role_definition_name = "Key Vault Secrets User"
  principal_id         = module.function_app_sedes.principal_id
}
...
resource "azurerm_role_assignment" "storage_data_sedes" {
  for_each             = local.storage_data_roles
  scope                = module.storage_sedes.id
